...
# ==== EDIT/DELETE GROUP MESSAGE ROUTES ====
...
def edit_message(message_id):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE tbl_messages SET message_content = ?, is_edited = 1 WHERE message_ID = ?",
            (new_content, message_id)
        )
        conn.commit()

        # Fetch updated message
        message = conn.execute('SELECT * FROM tbl_messages WHERE message_ID = ?', (message_id,)).fetchone()
        if not message:
...
def delete_message(message_id):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE tbl_messages SET message_content = '[deleted]', is_deleted = 1 WHERE message_ID = ?",
            (message_id,)
        )
        conn.commit()
        ...
        message = conn.execute('SELECT * FROM tbl_messages WHERE message_ID = ?', (message_id,)).fetchone()
        if not message:
            print(f"ERROR: Message with ID {message_id} not found after delete.")
            return jsonify({"error": "Message not found"}), 404
...
def send_private_message():
    try:
        ...
        message_text = request.form.get('content') or ''
        current_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # Handle multiple files (client may send multiple 'file' entries)
            ...
            for file in files:
                if file and allowed_file(file.filename):
                    ...
                    if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.webp')):
                        attachments.append(f"file:image|{file_url}|{filename}")
                    else:
                        attachments.append(f"file:other|{file_url}|{filename}")

        # Compose message_content: text part (may be empty) then '||' + attachments joined by '::' if any attachments
        if attachments:
            if message_text:
                message_content = f"{message_text}||{'::'.join(attachments)}"
            else:
                message_content = f"||{'::'.join(attachments)}"
        else:
        ...
        # Emit into the '/direct' namespace rooms for sender and receiver
        socketio.emit('receive_private_message', response_data, room=str(sender_user_id), namespace='/direct')
        socketio.emit('receive_private_message', response_data, room=str(receiver_user_id), namespace='/direct')
...
# ==== EDIT/DELETE PRIVATE MESSAGE ROUTES ====

# Edit private message
@app.route('/private_messages/<int:message_id>', methods=['PATCH'])
def edit_private_message(message_id):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE tbl_private_messages SET message_content = ?, is_edited = 1 WHERE message_ID = ?",
            (new_content, message_id)
        )
        conn.commit()

        # Fetch updated message
        message = conn.execute('SELECT * FROM tbl_private_messages WHERE message_ID = ?', (message_id,)).fetchone()
        if not message:
            return jsonify({"error": "Message not found"}), 404

        message_dict = dict(message)
        # Emit to both sender and receiver rooms for real-time update on '/direct' namespace
        socketio.emit('private_message_updated', {
            'message_ID': message_id,
            'sender_ID': message_dict['sender_ID'],
            'receiver_ID': message_dict['receiver_ID'],
            'new_content': new_content,
            'is_edited': True
        }, room=str(message_dict['sender_ID']), namespace='/direct')
        socketio.emit('private_message_updated', {
            'message_ID': message_id,
            'sender_ID': message_dict['sender_ID'],
            'receiver_ID': message_dict['receiver_ID'],
            'new_content': new_content,
            'is_edited': True
        }, room=str(message_dict['receiver_ID']), namespace='/direct')
...
@app.route('/private_messages/<int:message_id>', methods=['DELETE'])
def delete_private_message(message_id):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE tbl_private_messages SET message_content = '[deleted]', is_deleted = 1 WHERE message_ID = ?",
            (message_id,)
        )
        conn.commit()
...
def get_private_messages():
    ...
    try:
        query = '''
        SELECT pm.message_ID, pm.sender_ID, pm.receiver_ID, pm.message_content, pm.message_timestamp,
               pm.message_read, COALESCE(s.student_name, st.staff_name) as sender_name,
               COALESCE(pm.is_edited, 0) as is_edited, COALESCE(pm.is_deleted, 0) as is_deleted
...
# === Real-time edit/delete handlers for chat ===
@socketio.on('edit_message')
def handle_edit_message(data):
    message_id = data.get('message_ID')
    new_content = data.get('new_content')
    channel_id = data.get('channelId')
    if not message_id or not new_content or not channel_id:
        return
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        ...
        print(f"Socket edit_message error: {e}")
        conn.rollback()
    finally:
        conn.close()
...
def handle_delete_message(data):
    ...
    try:
        ...
        print(f"Socket delete_message error: {e}")
        conn.rollback()
    finally:
        conn.close()

@socketio.on('private_message_updated')
def handle_private_message_updated(data):
    # Broadcast to sender and receiver
    emit('private_message_updated', data, room=str(data['sender_ID']))
    emit('private_message_updated', data, room=str(data['receiver_ID']))

@socketio.on('private_message_deleted')
def handle_private_message_deleted(data):
    # Broadcast to sender and receiver
    emit('private_message_deleted', data, room=str(data['sender_ID']))
    emit('private_message_deleted', data, room=str(data['receiver_ID']))
