...
const StartPage = () => {
  ...
  return (
    <div className='grid grid-cols-12 gap-4 p-4 h-[calc(100vh)] bg-gradient-to-b from-background to-background-secondary'>
      <div className='col-start-1 col-span-3 p-4 flex flex-col text-text bg-primary shadow-lg rounded-3xl overflow-hidden'>
        ...
        <div className='bg-tertiary overflow-y-auto border-2 rounded-3xl border-accent' style={{ maxHeight: '250px' }}>
          {modules.map(module => (
            <button
              key={module.module_ID}
              onClick={() => handleModuleClick(module.module_ID)}
              className='text-lg active:bg-button-sec active:text-black focus:bg-button-sec focus:text-black z-50 shadow-lg my-2 py-1 px-3 flex items-center justify-center bg-secondary hover:bg-white hover:text-black rounded-full transition-all duration-200 ease-linear cursor-pointer text-text'
      ...
      <div className='col-start-4 col-span-9 flex flex-col text-white gap-4 mr-2 text-text'>
        <div className='bg-primary h-1/8 flex items-center justify-center p-4 rounded-3xl text-text'>
          <h1 className='text-4xl font-bold truncate'>
            {selectedChannelId ? channels.find(ch => ch.channel_ID.toString() === selectedChannelId)?.channel_name : 'Select a channel'}
          </h1>
        </div>
        <div className='bg-primary h-full rounded-3xl overflow-hidden p-4 text-text flex flex-col'>
          {selectedChannelId ? (
            <Chat channelId={selectedChannelId} />
          ) : (
            <div className='text-center text-text text-2xl font-bold flex-grow flex items-center justify-center'>
